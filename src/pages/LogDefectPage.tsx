...
import { CheckCircle2, Info, HelpCircle, AlertTriangle, Upload, X, ChevronDown, Search } from 'lucide-react';
  ...
  return (
    <Layout title={t('logDefect.title')}>
      <div className="mx-auto max-w-2xl">
        <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">{t('logDefect.subtitle')}</p>
        <Card>
          <CardBody className="space-y-4">
            ...
            <div>
              <label htmlFor="defectCode" className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
              ...
              {!newCodeMode ? (
                <div className="relative">
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 pr-8 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                      />
                  ...
                  {codeDropdownOpen && (
                    <div className="absolute z-10 mt-1 max-h-48 w-full overflow-auto rounded-md border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-800">
                      {filteredCodes.length === 0 ? (
                        <p className="px-3 py-2 text-xs text-slate-400">No codes found</p>
                      ) : (
                        filteredCodes.map((c) => (
                          <button
                            key={c.code}
                            ...
                            className="block w-full px-3 py-2 text-left hover:bg-slate-100 focus:outline-none dark:hover:bg-slate-700"
                          >
                            <span className="font-mono text-xs font-semibold text-slate-700 dark:text-slate-300">{c.code}</span>
                            <span className="ml-2 text-xs text-slate-500 dark:text-slate-400">{c.description}</span>
                          </button>
                        ))
                      )}
                    </div>
            ...
            <div>
              ...
              {photoDataUrl ? (
                <div className="relative inline-block">
                  <img src={photoDataUrl} alt="Preview" className="max-h-40 rounded-md border border-slate-200 dark:border-slate-700" />
                  <button
                ...
                <button
                  ...
                  className="flex w-full flex-col items-center gap-1 rounded-md border border-dashed border-slate-300 py-6 text-slate-500 hover:border-slate-400 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-600 dark:hover:bg-slate-700/30"
                >
      ...
      <CardBody className="space-y-4 py-6">
        ...
        {result.classification === 'KNOWN' && result.knownFix && (
          <div className="space-y-3">
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('logDefect.result.known.probableCause')}</p>
              <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{result.knownFix.probableCause}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('logDefect.result.known.documentedFix')}</p>
              <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{result.knownFix.documentedFix}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('logDefect.result.known.safetyNote')}</p>
              <p className="mt-1 text-sm text-orange-700 dark:text-orange-300">{result.knownFix.safetyNote}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('common.owner')}</p>
              <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{result.knownFix.owner}</p>
            </div>
        ...
        {result.classification === 'SYSTEMIC' && (
          <div className="space-y-3">
            ...
            {result.recommendedAction && (
              <div>
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('logDefect.result.systemic.recommendedAction')}</p>
                <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{result.recommendedAction}</p>
              </div>
            )}
