...
export function DashboardPage() {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      setMetrics(m);
      setAlerts(a.filter((al) => al.state !== 'RESOLVED'));
      setActivity(act);
      setStationRisk(risk);
      setTrend(tr);
      setError(false);
      ...
      <div className="space-y-4">
                  ...
                  >
                    <Card className="transition-all group-hover:shadow-card-hover group-hover:-translate-y-0.5">
                      <CardBody>
                        ...
                        <p className="mt-2.5 text-3xl font-bold tabular-nums tracking-tight text-slate-800 dark:text-slate-100">
                          {card.value ?? 0}
                        </p>
                        <div className="mt-1 flex items-center gap-1 text-xs text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <Card className="lg:col-span-2">
            <CardHeader title={t('dashboard.activeAlerts')} action={<span className="text-xs text-slate-400">{alerts.length}</span>} />
            {loading ? (
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {alerts.map((a) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 font-mono tabular-nums text-xs text-slate-500 dark:text-slate-400">{formatTimeShort(a.firstOccurrence)}</td>
                        <td className="px-3 py-2 font-mono tabular-nums text-xs text-slate-500 dark:text-slate-400">{formatTimeShort(a.latestOccurrence)}</td>
          ...
          <Card>
            ...
            <CardBody>
              {loading ? (
                ...
                <div className="grid grid-cols-5 gap-2">
                  {stationRisk.map((s) => {
                    const ratio = s.count / s.threshold;
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border p-2.5 text-center transition-all hover:shadow-card-hover hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-blue-500 ${bg} ${
                          selectedStation === s.stationId ? 'ring-2 ring-blue-500 ring-offset-1 dark:ring-offset-slate-800' : ''
                        }`}
                      >
                        <p className="font-mono text-xs font-semibold text-slate-700 dark:text-slate-300">{s.stationId}</p>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardBody>
              {loading ? (
                ...
                <EmptyState message={t('dashboard.noActivity')} icon={<ActivityIcon className="h-8 w-8 text-slate-300 dark:text-slate-600" />} />
              ) : (
                <ul className="space-y-2">
                  {activity.map((ev) => (
                    <li key={ev.id} className="flex items-center gap-3 text-sm">
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700">
                        <ActivityIcon className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" />
                      </span>
                      <span className="flex-1 text-slate-700 dark:text-slate-300">{ev.message}</span>
                      <span className="font-mono tabular-nums text-xs text-slate-400">{formatTime(ev.timestamp)}</span>
                    </li>
                  ))}
                </ul>
              )}
