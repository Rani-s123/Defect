...
import { ChevronRight, BookOpen, Clock, ImageIcon, ArrowRight, X } from 'lucide-react';
...
export function AlertDetailPage() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    try {
      const [a, ev, hist, fix] = await Promise.all([
        api.getAlert(id),
        api.getEvidence(id),
        api.getHistory(id),
        api.getKnownFix(''),
    ...
    <Layout title={t('alertDetail.title')}>
      <div className="space-y-4">
        ...
        <Card>
          <CardBody className="space-y-2">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="font-mono text-sm font-semibold text-slate-700 dark:text-slate-300">{alert.id}</span>
            </div>
            <div className="flex flex-wrap gap-4 text-sm text-slate-600 dark:text-slate-400">
              <span><strong className="text-slate-500">{t('common.station')}:</strong> <span className="font-mono">{alert.stationId}</span></span>
              <span><strong className="text-slate-500">{t('common.code')}:</strong> <span className="font-mono">{alert.defectCode}</span></span>
              {alert.ticketId && <span><strong className="text-slate-500">Ticket:</strong> <span className="font-mono">{alert.ticketId}</span></span>}
              {alert.ncrId && <span><strong className="text-slate-500">NCR:</strong> <span className="font-mono">{alert.ncrId}</span></span>}
            </div>
          </CardBody>
        </Card>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <ol className="relative space-y-4 border-l border-slate-200 pl-4 dark:border-slate-700">
                  {evidence.map((d) => (
                    <li key={d.id} className="relative">
                      <span className="absolute -left-[21px] top-1 flex h-3 w-3 items-center justify-center rounded-full border-2 border-white bg-slate-400 dark:border-slate-800" />
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs font-semibold text-slate-700 dark:text-slate-300">{d.serialNumber}</span>
                          <span className="font-mono tabular-nums text-xs text-slate-400">{formatTime(d.occurredAt)}</span>
                        </div>
                        <p className="text-xs text-slate-600 dark:text-slate-400">{d.description}</p>
                        <p className="text-xs text-slate-400">{t('common.actor')}: {d.loggedBy}</p>
                        {d.photoDataUrl && (
                          <button
          ...
          {/* Known fix */}
          <Card>
            <CardHeader title={t('alertDetail.knownFix')} action={<BookOpen className="h-4 w-4 text-slate-400" />} />
            <CardBody>
              {knownFix === undefined ? (
                <Skeleton className="h-20 w-full" />
              ) : knownFix === null ? (
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('kb.probableCause')}</p>
                    <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{knownFix.probableCause}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('kb.documentedFix')}</p>
                    <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{knownFix.documentedFix}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('kb.safetyNote')}</p>
                    <p className="mt-1 text-sm text-orange-700 dark:text-orange-300">{knownFix.safetyNote}</p>
                  </div>
                  <p className="text-xs text-slate-400">{t('common.owner')}: {knownFix.owner}</p>
                </div>
              )}
            </CardBody>
          </Card>
                  ...
                  <div key={s} className="flex items-center gap-1">
                    ...
                    >
                      <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${
                        isCurrent ? 'bg-blue-600 text-white' : isDone ? 'bg-green-600 text-white' : 'bg-slate-200 text-slate-500 dark:bg-slate-700'
                      }`}>
                        {i + 1}
                      </span>
                      {t(`state.${s}`)}
                    </div>
                    {i < STATE_ORDER.length - 1 && <ChevronRight className="h-3 w-3 text-slate-300 dark:text-slate-600" />}
                  </div>
                );
              })}
            </div>
        ...
        {alert.ncrId && (
          <Card>
            <CardHeader title={t('alertDetail.ncrSection')} action={<span className="font-mono text-xs text-slate-400">{alert.ncrId}</span>} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {history.map((h) => (
                      <tr key={h.id}>
                        <td className="py-2 pr-3 text-slate-700 dark:text-slate-300">{h.actor}</td>
                        <td className="py-2 pr-3 font-mono tabular-nums text-xs text-slate-500 dark:text-slate-400">{formatTime(h.timestamp)}</td>
                        <td className="py-2 pr-3">
                          <span className="font-mono text-xs text-slate-600 dark:text-slate-400">
                            {h.fromState !== '—' ? t(`state.${h.fromState}`) : '—'} → {t(`state.${h.toState}`)}
                          </span>
                        </td>
                        <td className="py-2 text-xs text-slate-600 dark:text-slate-400">{h.note}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* Photo modal */}
      {photoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 p-4" onClick={() => setPhotoModal(null)}>
          <img src={photoModal} alt="Defect photo" className="max-h-[80vh] max-w-full rounded-lg" />
          <button className="absolute right-4 top-4 rounded-full bg-white p-2 text-slate-700 hover:bg-slate-100" aria-label="Close">
            <X className="h-5 w-5" />
          </button>
        </div>
      )}
