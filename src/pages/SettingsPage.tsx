...
export function SettingsPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.saveSettings(Number(thresholdN), Number(windowT));
      setSaved(true);
  ...
  return (
    <Layout title={t('settings.title')}>
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">{t('settings.subtitle')}</p>
      ...
      {loading ? (
        ...
        <div className="max-w-lg space-y-4">
          <Card>
            <CardHeader title={t('settings.title')} action={<SettingsIcon className="h-4 w-4 text-slate-400" />} />
